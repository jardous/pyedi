...
class QSci(qs):
    ...
    def setupContextMenu(self):
        ...
        main_window = qApp.mainWidget()
        
        self.menuIds['New'] = self.file.insertItem('&New', main_window.newDocAction, Qt.CTRL + Qt.Key_N)
        self.menuIds['Open'] = self.file.insertItem('&Open', main_window.openFile, Qt.CTRL + Qt.Key_O)
        ...
        self.menuIds['Print'] = self.file.insertItem('&Print', main_window.printDoc, Qt.CTRL + Qt.Key_P)
        self.file.insertSeparator()
        self.menuIds['Close'] = self.file.insertItem('&Close', main_window.closeCurrentDoc,  Qt.CTRL + Qt.Key_W)
    ...
    def save(self):
        try:
            ...
            self.emit(PYSIGNAL('status_message'), ('Can not write to %s' % (self.filename), 2000))
            return
        ...
        self.setModified(False)
        self.setTabLabel()
        self.emit(PYSIGNAL('status_message'), ('File %s saved' % (self.filename), 2000))
    
    def saveAs(self):
        fn = QFileDialog.getSaveFileName(self.filename, '', self)
        if not fn.isEmpty():
...
class ApplicationWindow(QMainWindow):
    ...
    def newDocAction(self):
        self.newDoc()
    
    def newDoc(self, filename='Untitled.txt'):
